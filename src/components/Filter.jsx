...
const Filter = () => {
  ...
  return (
    <div className='flex flex-row justify-end px-2 mb-1'>
        ...
        <button className='px-1 ml-2 font-bold text-white bg-green-600 hover:bg-green-400 disabled:bg-gray-400 disabled:cursor-not-allowed rounded' onClick={ handleClick } disabled={disabled}>Filter</button>

        { showLogout ? <Link className='px-1 ml-2 font-bold text-white bg-orange-600 hover:bg-orange-400 rounded' href='/posts/logout'>Logout</Link> : ''}
    </div>
  )
}
